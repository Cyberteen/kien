...
//update:24-07-16: 1.Obtaining stable weight is consolidated inside a function for multiple calls. 2.Real-time weight update functionality added
//update:30-07-16: 1.Tare button added and functonality working checked.
...
HX711 scale(6, 7);		// parameter "gain" is ommited; the default value 128 is used by the library
float avg =0.0,currentWeight=0.0,lastWeight=0.0;
boolean upload = true;
unsigned long int up;
const int tarePin = 2,setupPin=11;
//int calib = 23100;
...
void sendWeight(String msg) // for sending weight to esp8266 via serial
{
...
// ideal 19500
  scale.set_scale(19500);                      // this value is obtained by calibrating the scale with known weights; see the README for details
...
//    }
  ...
  {
    Serial.println("Setup pin pressed");
    boolean flag= false;
    ...
    if(count==5)
    {
      ...
      lcd.clear();
      lcd.setCursor(0,0);
      lcd.print("Enter WiFi setup?");
      lcd.print(0,1);
      lcd.print("Press within 15s");
      t1=millis();
      count=15;
      flag=false;
    ...
    {
      Serial.println("Less than 5 counts!");
      Serial.print("count:");
      Serial.println(count);
    }
  }
